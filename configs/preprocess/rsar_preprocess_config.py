...
from jrs.models.boxes.box_ops import poly_to_rotated_box_single
...
def convert_data_to_mmdet(src_path, out_path, trainval=True, filter_empty_gt=True, ext='.png', type=''):
    ...
    label_path = os.path.join(src_path, 'annfiles')
    img_lists = os.listdir(img_path)
    img_lists.sort()

    data_dict = []
    for img_file_path in tqdm(img_lists):
        img_info = {}
        img_name = osp.splitext(img_file_path)[0]
        label = os.path.join(label_path, img_name + '.txt')
        img = Image.open(osp.join(img_path, img_file_path))
        img_info['filename'] = osp.basename(img_file_path)
...
RSAR_PATH = "/RSAR_DATA"
for sub_type in ['train', 'val', 'test']:
    src_path = osp.join(RSAR_PATH, sub_type)
    assert osp.exists(src_path)
    convert_data_to_mmdet(src_path=src_path, out_path=osp.join(src_path, 'labels.pkl'), type='RSAR')