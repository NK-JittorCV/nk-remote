...
from jrs.utils.registry import MODELS,build_from_cfg,BACKBONES,HEADS,NECKS
import numpy as np
import jrs
import copy
...
class RetinaNet(nn.Module):
    ...
    def draw(self, images, results, targets, out_path):
        for i in range(images.shape[0]):
            ...
            jrs.utils.visualization.visualize_r_result(img.copy(), result, out_path)
            break
    ...
    def execute(self,images,targets):
        ...
        # jrs.utils.visualization.visualize_r_result(img.copy(), results[0])
        # print(losses)
        # exit(0)
        if self.is_training():
            return losses 
